...
#include "include/dx9/imgui_impl_dx9.h"
#include "include/win32/imgui_impl_win32.h"
...
static HRESULT __stdcall present(IDirect3DDevice9* device, const RECT* src, const RECT* dest, HWND windowOverride, const RGNDATA* dirtyRegion) noexcept
{
    ...
    if (!initialize)
    {
        ...
        std::string sFontPath = +"C:\\Windows\\Fonts\\verdana.ttf";
        ...
        g_pMenu->InitTextures(device);
        g_pMenu->m_pThemes->SetDefaultDarkTheme();
        initialize = true;
    }
...
static void swapWindow(SDL_Window * window) noexcept
{
    ...
    g_pMenu->m_bIsOpened = gui->isOpen();
...
static bool __STDCALL createMove(LINUX_ARGS(void* thisptr,) float inputSampleTime, UserCmd* cmd) noexcept
{
    ...
    Misc::autoStrafe(cmd);
    Misc::MultidirectionalStrafe(cmd, cmd->viewangles);
