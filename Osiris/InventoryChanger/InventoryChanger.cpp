...
void anothetr_exam(ImVec2 button_size, bool& isInAddMode)
{
   

    if (!isInAddMode && ImGui::Button("Add items..", button_size))
        isInAddMode = true;
...
void _1_another_exam(ImVec2 button_size, bool& isInAddMode, ImVec2 child_size)
{
    ...
    if (isInAddMode) {
        ...
        if (ImGui::Button(("Add selected (" + std::to_string(selectedToAdd.size()) + ")").c_str(), button_size)) {
            isInAddMode = false;
        ...
        if (ImGui::BeginChild("##scrollarea", ImVec2{ child_size.x - 20,  child_size.y - 100 }))
        {
