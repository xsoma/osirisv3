...
void GUI::updateColors() const noexcept
{
    ...
    case 0:  g_pMenu->m_pThemes->SetDefaultDarkTheme(); break;
...
{
    static int currentCategory{ 0 };
    const char* t[] = { "All", "Pistols", "Heavy", "SMG", "Rifles", "Zeus x27" };
    g_pMenu->m_pGui->Combo(&currentCategory, "Category", t, IM_ARRAYSIZE(t));
    ...
    g_pMenu->m_pGui->Checkbox(&config->triggerbot[currentWeapon].enabled, "Enabled");
    g_pMenu->m_pGui->KeyToggle(&config->triggerbotHoldKey, "Hold Key");
    g_pMenu->m_pGui->Checkbox(&config->triggerbot[currentWeapon].friendlyFire, "Friendly fire");
    g_pMenu->m_pGui->Checkbox(&config->triggerbot[currentWeapon].scopedOnly, "Scoped only");
    g_pMenu->m_pGui->Checkbox(&config->triggerbot[currentWeapon].ignoreFlash, "Ignore flash");
    g_pMenu->m_pGui->Checkbox(&config->triggerbot[currentWeapon].ignoreSmoke, "Ignore smoke");



    const char* tt[] = { "All", "Head", "Chest", "Stomach", "Left arm", "Right arm", "Left leg", "Right leg"};
    g_pMenu->m_pGui->Combo(&config->triggerbot[currentWeapon].hitgroup, "Hitgroup", tt, IM_ARRAYSIZE(tt));
    g_pMenu->m_pGui->Slider(&config->triggerbot[currentWeapon].shotDelay,"Shoot delay", 0, 250, "%d ms");
    ImGui::InputInt("Min dmg", &config->triggerbot[currentWeapon].minDamage);
    config->triggerbot[currentWeapon].minDamage = std::clamp(config->triggerbot[currentWeapon].minDamage, 0, 250);

    g_pMenu->m_pGui->Checkbox(&config->triggerbot[currentWeapon].killshot, "Killshot");

    g_pMenu->m_pGui->Slider(&config->triggerbot[currentWeapon].burstTime, "Burst Time", 0.0f, 0.5f, "%.3f s", 1.0f);
...
void new_chams_2()
{
    auto& chams{ config->chams["Local player"].materials[0] };
    g_pMenu->m_pGui->Checkbox(&chams.enabled, "Enable");
    const char* t[] = { "Normal", "Flat", "Animated", "Platinum", "Glass", "Chrome","Crystal", "Silver", "Gold", "Plastic", "Glow", "Pearlescent" ,"Metallic" };
    g_pMenu->m_pGui->Combo(&chams.material, "Material", t, IM_ARRAYSIZE(t));

    g_pMenu->m_pGui->Checkbox(&chams.healthBased, "Health based");
    g_pMenu->m_pGui->Checkbox(&chams.ignorez, "Ignore-Z");
    g_pMenu->m_pGui->Checkbox(&chams.cover, "Cover");
    g_pMenu->m_pGui->Checkbox(&chams.blinking, "Pulse");
...
void new_chams_3()
{
    auto& chams{ config->chams["Weapons"].materials[0] };
    g_pMenu->m_pGui->Checkbox(&chams.enabled, "Enable");
    const char* t[] = { "Normal", "Flat", "Animated", "Platinum", "Glass", "Chrome","Crystal", "Silver", "Gold", "Plastic", "Glow", "Pearlescent" ,"Metallic" };
    g_pMenu->m_pGui->Combo(&chams.material, "Material", t, IM_ARRAYSIZE(t));
...
void new_chams_4()
{
    auto& chams{ config->chams["Hands"].materials[0] };
    g_pMenu->m_pGui->Checkbox(&chams.enabled, "Enable");
    const char* t[] = { "Normal", "Flat", "Animated", "Platinum", "Glass", "Chrome","Crystal", "Silver", "Gold", "Plastic", "Glow", "Pearlescent" ,"Metallic" };
    g_pMenu->m_pGui->Combo(&chams.material, "Material", t, IM_ARRAYSIZE(t));
...
void new_chams_5()
{
    auto& chams{ config->chams["Sleeves"].materials[0] };
    g_pMenu->m_pGui->Checkbox(&chams.enabled, "Enable");
    const char* t[] = { "Normal", "Flat", "Animated", "Platinum", "Glass", "Chrome","Crystal", "Silver", "Gold", "Plastic", "Glow", "Pearlescent" ,"Metallic" };
    g_pMenu->m_pGui->Combo(&chams.material, "Material", t, IM_ARRAYSIZE(t));
...
void new_chams_1()
{
    auto& chams{ config->chams["Enemies"].materials[0] };
    g_pMenu->m_pGui->Checkbox(&chams.enabled, "Enable");
    const char* t[] = { "Normal", "Flat", "Animated", "Platinum", "Glass", "Chrome","Crystal", "Silver", "Gold", "Plastic", "Glow", "Pearlescent" ,"Metallic" };
    g_pMenu->m_pGui->Combo(&chams.material, "Material",t, IM_ARRAYSIZE(t));
...
void new_chams_6()
{
    auto& chams{ config->chams["Backtrack"].materials[0] };
    g_pMenu->m_pGui->Checkbox(&chams.enabled, "Enable");
    const char* t[] = { "Normal", "Flat", "Animated", "Platinum", "Glass", "Chrome","Crystal", "Silver", "Gold", "Plastic", "Glow", "Pearlescent" ,"Metallic" };
    g_pMenu->m_pGui->Combo(&chams.material, "Material", t, IM_ARRAYSIZE(t));
...
void thats_what_i_say(int& currentConfig, bool& incrementalLoad, std::vector<std::u8string>& configItems, std::u8string& buffer, ImVec2 button_size)
{
    if (ImGui::BeginPopup("Config to reset")) {
        ...
        for (int i = 0; i < IM_ARRAYSIZE(names); i++) {
            ...
            if (ImGui::Selectable(names[i])) {
                switch (i) {
                case 0: config->reset(); gui->updateColors(); Misc::updateClanTag(true); InventoryChanger::scheduleHudUpdate(); break;
                ...
                case 11: config->style = { }; gui->updateColors(); break;
    ...
    if (currentConfig != -1) {
        if (ImGui::Button("Load selected", button_size)) {
        ...
        if (ImGui::Button("Delete selected", button_size)) {
...
{
    ImGui::Begin("Hook", nullptr, windowFlags | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize);
