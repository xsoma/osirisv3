...
static void renderBox(const BoundingBox& bbox, const Box& config) noexcept
{
    ...
    case Box::_2d:
        if (config.fill.enabled)
            drawList->AddRectFilled(bbox.min + ImVec2{ 1.0f, 1.0f }, bbox.max - ImVec2{ 1.0f, 1.0f }, fillColor, config.rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight);
        else
            drawList->AddRect(bbox.min + ImVec2{ 1.0f, 1.0f }, bbox.max + ImVec2{ 1.0f, 1.0f }, color & IM_COL32_A_MASK, config.rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight);
        drawList->AddRect(bbox.min, bbox.max, color, config.rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight);
        break;
    case Box::_2dCorners: {
        if (config.fill.enabled) {
            drawList->AddRectFilled(bbox.min + ImVec2{ 1.0f, 1.0f }, bbox.max - ImVec2{ 1.0f, 1.0f }, fillColor, config.rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight);
...
void StreamProofESP::aimw_g_22()
{
    ImGuiCustom::colorPicker("Box color", config->streamProofESP.weapons["All"].box);
    ImGuiCustom::colorPicker("Name color", config->streamProofESP.weapons["All"].name);

}

void StreamProofESP::aimw_g_222()
{
    ImGuiCustom::colorPicker("Box color", config->streamProofESP.projectiles["All"].box);
    ImGuiCustom::colorPicker("Name color", config->streamProofESP.projectiles["All"].name);

}


void StreamProofESP::aimw_g_2()
{
    ImGuiCustom::colorPicker("Box color", config->streamProofESP.enemies["All"].box);

    ImGuiCustom::colorPicker("Head box color", config->streamProofESP.enemies["All"].headBox);
    ImGuiCustom::colorPicker("Skeleton color", config->streamProofESP.enemies["All"].skeleton);
    ImGuiCustom::colorPicker("Snapline color", config->streamProofESP.enemies["All"].snapline);

    ImGuiCustom::colorPicker("Weapon color", config->streamProofESP.enemies["All"].weapon);



}
void StreamProofESP::aimw_g_1()
{

    const char* types[] = {"2D", "2D Corners", "3D", "3D Corners"};
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].enabled, "Enable");
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].box.enabled, "Box");
    g_pMenu->m_pGui->Combo(&config->streamProofESP.enemies["All"].box.type, "Box type", types, IM_ARRAYSIZE(types));

    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].name.enabled, "Name");
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].headBox.enabled, "Head box");
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].healthBar.enabled, "Health");

    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].skeleton.asColorToggle().enabled, "Skeleton");

    g_pMenu->m_pGui->Slider(&config->streamProofESP.enemies["All"].skeleton.thickness, "Skeleton thickness", 1.f, 20.f, "%.0f", 1.f);

    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].snapline.asColorToggle().enabled, "Snapline");

    const char* ztypes[] = { "Bottom", "Top", "Crosshair" };
    g_pMenu->m_pGui->Combo(&config->streamProofESP.enemies["All"].snapline.type, "Snapline type", ztypes, IM_ARRAYSIZE(ztypes));
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.enemies["All"].weapon.enabled, "Weapon");
...
void StreamProofESP::aimw_g_3()
{
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.weapons["All"].enabled, "Enable");
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.weapons["All"].box.enabled, "Box");
    const char* types[] = { "2D", "2D Corners", "3D", "3D Corners" };
    g_pMenu->m_pGui->Combo(&config->streamProofESP.weapons["All"].box.type, "Box type", types, IM_ARRAYSIZE(types));
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.weapons["All"].name.enabled, "Name");
  
}


void StreamProofESP::aimw_g_4()
{
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.projectiles["All"].enabled, "Enable");
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.projectiles["All"].box.enabled, "Box");
    const char* types[] = { "2D", "2D Corners", "3D", "3D Corners" };
    g_pMenu->m_pGui->Combo(&config->streamProofESP.projectiles["All"].box.type, "Box type", types, IM_ARRAYSIZE(types));
    g_pMenu->m_pGui->Checkbox(&config->streamProofESP.projectiles["All"].name.enabled, "Name");
