...
void Misc::aimware_gui_2() noexcept
{
    ...
    g_pMenu->m_pGui->Checkbox(&miscConfig.bunnyHop, "Bunny hop", "Jumping...");
    g_pMenu->m_pGui->Checkbox(&miscConfig.autoStrafe, "Auto strafe", "auto strafe...");
    g_pMenu->m_pGui->Checkbox(&miscConfig.wasdHop, "Multidirectional auto strafe", "wasd strafe...");
    g_pMenu->m_pGui->Checkbox(&miscConfig.antiAfkKick, "Anti AFK kick");

    g_pMenu->m_pGui->Checkbox(&miscConfig.fastDuck, "Fast duck");
    g_pMenu->m_pGui->Checkbox(&miscConfig.moonwalk, "Moonwalk");
    g_pMenu->m_pGui->Checkbox(&miscConfig.edgejump, "Edge Jump");

    g_pMenu->m_pGui->KeyToggle(&miscConfig.edgejumpkey, "Edge Jump Key");

    g_pMenu->m_pGui->Checkbox(&miscConfig.slowwalk, "Slowwalk");
    g_pMenu->m_pGui->KeyToggle(&miscConfig.slowwalkKey, "Slowwalk Key");





    g_pMenu->m_pGui->Checkbox(&miscConfig.autoPistol, "Auto pistol");
    g_pMenu->m_pGui->Checkbox(&miscConfig.autoReload, "Auto reload");
    g_pMenu->m_pGui->Checkbox(&miscConfig.autoAccept, "Auto accept");

    g_pMenu->m_pGui->Checkbox(&miscConfig.radarHack, "Radar hack");
    g_pMenu->m_pGui->Checkbox(&miscConfig.revealRanks, "Reveal ranks");
    g_pMenu->m_pGui->Checkbox(&miscConfig.revealMoney, "Reveal money");
    g_pMenu->m_pGui->Checkbox(&miscConfig.revealSuspect, "Reveal suspect");
    g_pMenu->m_pGui->Checkbox(&miscConfig.revealVotes, "Reveal votes");

    g_pMenu->m_pGui->Checkbox(&miscConfig.spectatorList.enabled, "Spectator list");
...
void Misc::aimware_gui_4() noexcept
{

    g_pMenu->m_pGui->Checkbox(&miscConfig.watermark.enabled, "Watermark");
    g_pMenu->m_pGui->Checkbox(&miscConfig.fixAnimationLOD, "Fix animation LOD");
    g_pMenu->m_pGui->Checkbox(&miscConfig.fixBoneMatrix, "Fix bone matrix");
    g_pMenu->m_pGui->Checkbox(&miscConfig.fixMovement, "Fix movement");
    g_pMenu->m_pGui->Checkbox(&miscConfig.disableModelOcclusion, "Disable model occlusion");
    g_pMenu->m_pGui->Checkbox(&miscConfig.disablePanoramablur, "Disable HUD blur");


    g_pMenu->m_pGui->Slider(&miscConfig.aspectratio, "Aspect Ratio", 0.f, 5.f, "%.2f°", 1.f);


}
#include "../imgui/imgui_stdlib.h"
void Misc::aimware_gui_5(float width, ImVec2 button_size) noexcept
{

    g_pMenu->m_pGui->Checkbox(&miscConfig.animatedClanTag, "Animated clan tag");
    g_pMenu->m_pGui->Checkbox(&miscConfig.clocktag, "Clock tag");
    g_pMenu->m_pGui->Checkbox(&miscConfig.customClanTag, "Custom clantag");
    ...
    ImGui::Spacing();
    g_pMenu->m_pGui->Checkbox(&miscConfig.killMessage, "Kill message");
    ImGui::SetNextItemWidth(width);
    ImGui::PushID(1);
    ...
    g_pMenu->m_pGui->Checkbox(&miscConfig.nameStealer, "Name stealer");

    const char* ban_color[] = { "White" , "Red" , "Purple" , "Green" , "Light green" , "Turquoise" , "Light red" , "Gray" , "Yellow" , "Gray 2" , "Light blue" , "Gray/Purple" , "Blue" , "Pink" , "Dark orange" , "Orange" };
    g_pMenu->m_pGui->Combo(&miscConfig.banColor, "Ban color", ban_color, IM_ARRAYSIZE(ban_color), "Ban color type.");
    ...
    if (ImGui::Button("Setup fake ban", button_size))
        Misc::fakeBan(true);


}
void Misc::aimware_gui_6(float width, ImVec2 button_size) noexcept
{


    g_pMenu->m_pGui->Checkbox(&miscConfig.nadePredict, "Grenade Prediction");

    g_pMenu->m_pGui->Checkbox(&miscConfig.fixTabletSignal, "Fix tablet signal");

    g_pMenu->m_pGui->Checkbox(&miscConfig.oppositeHandKnife, "Opposite Hand Knife");
    g_pMenu->m_pGui->Checkbox(&miscConfig.preserveKillfeed.enabled, "Preserve Killfeed");
    g_pMenu->m_pGui->Checkbox(&miscConfig.purchaseList.enabled, "Purchase List");
   


}
void Misc::aimware_gui_7() noexcept
{
    g_pMenu->m_pGui->Checkbox(&miscConfig.fastPlant, "Fast plant");
    g_pMenu->m_pGui->Checkbox(&miscConfig.fastStop, "Fast stop");
    g_pMenu->m_pGui->Checkbox(&miscConfig.quickReload, "Quick reload");
    g_pMenu->m_pGui->Checkbox(&miscConfig.prepareRevolver, "Prepare revolver");

    g_pMenu->m_pGui->KeyToggle(&miscConfig.prepareRevolverKey, "Revolver key");
...
void Misc::aimware_gui_1() noexcept
{
 
    g_pMenu->m_pGui->Checkbox(&miscConfig.blur_background, "Blur background", "Blur smooth effect when enable/disable menu");
...
void Misc::aimware_gui_3() noexcept
{
    
    g_pMenu->m_pGui->KeyToggle(&miscConfig.menuKey, "Menu Key");
...
void DrawShapeAnimated(const char* label, ImVec2 min, ImVec2 max, ImVec2 pos, ImColor color) {
    ...
    int hovered_delta = Animate(label, "hovered_rect", ImGui::IsMouseHoveringRect(min, max, false), 255.f, 15.f, STATIC);

    ImGui::SetCursorPos(pos - ImGui::GetWindowPos());

    auto clicked = ImGui::GetIO().MouseClicked[0] && ImGui::IsMouseHoveringRect(min, max, false);
    auto ID = ImGui::GetID(pChar(label, "b_animation").c_str());
    ...
    float radius = Animate(label, "circle_radius", ItPLibrary->second, 100.f, 10.f, DYNAMIC);
    ...
    ImGui::BeginChild(pChar("##drawlist_zone", label).c_str(), size, false); {
        if (ItPLibrary->second)
...
void create_window(const char* name, std::vector <std::string> vec) {
    ...
    GImGui->Style.WindowBorderSize = 0.f;
    if (ImGui::Begin(name, nullptr, dw_window_flags))
    {
       // PostProcessing::performFullscreenBlur(ImGui::GetWindowDrawList(), 1.f);
        ...
        //get vars :::
        ImColor theme = ImColor(242, 130, 45, 255);
        ImColor theme_zero = ImColor(242, 130, 45, 0);
        ImColor black = ImColor(0, 0, 0, 210);
        ImColor black_zero = ImColor(0, 0, 0, 0);
        ImColor black_half = ImColor(0, 0, 0, 60);

        //background
        draw->AddRectFilledMultiColor(p + ImVec2(10, 0), p + ImVec2(s.x / 2, s.y), black_half, black_half, black_half, black_half);
        draw->AddRectFilledMultiColor(p + ImVec2(s.x / 2, 0), p + ImVec2(s.x, s.y) - ImVec2(10, 0), black_half, black_half, black_half, black_half);
       ...
       // DrawShapeAnimated(name, shape_pos, shape_pos + shape_sz, shape_pos, theme);
        ...
        for (auto i = 0; i < vec.size(); i++) {
            int alpha_of_this = Animate(name, vec[i].c_str(), true, 255, 15, STATIC);
            auto first_circle_pos = ImVec2(p.x + 20, p.y + s.y + 15 + 25 * i);
            draw->AddCircleFilled(first_circle_pos, 2.3f, ImColor(242, 130, 45, alpha_of_this), 5.f * 15.f);
            draw->AddCircleFilled(first_circle_pos, 2.f, ImColor(242, 130, 45, alpha_of_this), 5.f * 15.f);
            draw->AddText(first_circle_pos + ImVec2(10, -9), ImColor(255, 255, 255, alpha_of_this), vec[i].c_str());
        }
...
void Misc::blur_in_all() noexcept
{
    static float option = 0.0f;
    if (miscConfig.blur_background)
    {
        if (gui->isOpen())
            option = std::min(option + ImGui::GetIO().DeltaTime * 2.5f, 1.0f);
        else
          option = std::max(option - ImGui::GetIO().DeltaTime * 2.5f, 0.0f);

        PostProcessing::performFullscreenBlur(ImGui::GetBackgroundDrawList(), option);
    }
    else
    {
        option = 0.0f;
...
void Misc::watermark() noexcept
{
    ...
    PostProcessing::performFullscreenBlur(ImGui::GetWindowDrawList(), 1.0f);
    static auto frameRate = 1.0f;
    frameRate = 0.9f * frameRate + 0.1f * memory->globalVars->absoluteFrameTime;

    ImGui::Text("Hook | %d fps | %d ms", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency());
    ImGui::End();
}
...
void Misc::MultidirectionalStrafe(UserCmd* cmd, Vector& orig) noexcept
{
    if (localPlayer
        && !miscConfig.wasdHop)
        return;
    ...
    if (cmd->buttons & UserCmd::IN_SPEED || !miscConfig.wasdHop || localPlayer->velocity().length2D() < 10.f)
        return;

    static float yaw_add = 0.f;
    static const auto cl_sidespeed = interfaces->cvar->findVar("cl_sidespeed");
    if (!(localPlayer->flags() & (1 << 0)))
    {
        bool back = cmd->buttons & UserCmd::IN_BACK;
        bool forward = cmd->buttons & UserCmd::IN_FORWARD;
        bool right = cmd->buttons & UserCmd::IN_MOVELEFT;
        bool left = cmd->buttons & UserCmd::IN_MOVERIGHT;
        ...
        const auto delta = zNormalizeYaw(orig.y - RAD2DEG(atan2(localPlayer->velocity().y, localPlayer->velocity().x)));

        cmd->sidemove = delta > 0.f ? -cl_sidespeed->getFloat() : cl_sidespeed->getFloat();

        orig.y = zNormalizeYaw(orig.y - delta);
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Hook\u2022 %c%s\x01 voted %c%s\x01", isLocal ? '\x01' : color, isLocal ? "You" : entity->getPlayerName().c_str(), color, votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const void* data, int size) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Hook\u2022 %c%s\x01 call vote (\x06%s\x01)", isLocal ? '\x01' : '\x06', isLocal ? "You" : entity->getPlayerName().c_str(), voteName(voteType));
}

void Misc::onVotePass() noexcept
{
    if (miscConfig.revealVotes)
        memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Hook\u2022\x01 Vote\x06 PASSED");
}

void Misc::onVoteFailed() noexcept
{
    if (miscConfig.revealVotes)
        memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Hook\u2022\x01 Vote\x07 FAILED");
}
...
static void from_json(const json& j, MiscConfig& m)
{
    ...
    read(j, "Blur background", m.blur_background);
    read(j, "wasd", m.wasdHop);
...
static void to_json(json& j, const MiscConfig& o)
{
    ...
    WRITE("Blur background", blur_background);
