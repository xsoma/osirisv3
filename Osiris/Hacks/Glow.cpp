...
void Glow::drawgl()
{
    ...
    const char* category[] = { "Allies", "Enemies", "Planting", "Defusing", "Local Player", "Weapons", "C4", "Planted C4", "Chickens", "Defuse Kits", "Projectiles", "Hostages", "Ragdolls" };
    g_pMenu->m_pGui->Combo(&currentCategory, "Category type", category, IM_ARRAYSIZE(category));
    ImGui::PopID();
    ...
    if (currentCategory <= 3) {
        ...
        ImGui::PushID(1);
        const char* type[] = { "All", "Visible", "Occluded" };
        g_pMenu->m_pGui->Combo(&currentType, "Type", type, IM_ARRAYSIZE(type));
        ImGui::PopID();
        auto& cfg = playerGlowConfig[category[currentCategory]];
        switch (currentType) {
        ...
        currentItem = &glowConfig[category[currentCategory]];
    }

    g_pMenu->m_pGui->Checkbox(&currentItem->enabled, "Enabled");
    g_pMenu->m_pGui->Checkbox(&currentItem->healthBased, "Health based");


    ImGuiCustom::colorPicker("Color", *currentItem);

 
    const char* t[] = { "Default", "Rim3", "Edge", "Edge Pulse"};
    g_pMenu->m_pGui->Combo(&currentItem->style, "Style", t, IM_ARRAYSIZE(t));
