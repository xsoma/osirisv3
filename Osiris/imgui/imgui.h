...
namespace ImGui
{
    ...
    bool BeginDefault(const char* name, bool* p_open, ImGuiWindowFlags flags = 0);
	...
	IMGUI_API void          Separator(int count);
	...
	IMGUI_API bool			ButtonTabs(const char* label, const ImVec2& size_arg = ImVec2(0, 0));
	IMGUI_API bool			ButtonTabsNormal(const char* label, const ImVec2& size_arg = ImVec2(0, 0));
    ...
    IMGUI_API bool          ColorEditFixed(const char* label, float& v0, float& v1, float& v2, float& v3);



	IMGUI_API bool			ColorEdit(const char* label, float* v);
	IMGUI_API bool			KeyButton(const char* label, void* v, bool owned = false);
	IMGUI_API bool			KeyButtonToggle(const char* label, void* v, bool owned = false);
	IMGUI_API bool			KeyButtonPress(const char* label, void* v, bool owned = false);

    // Widgets: Combo Box
    // - The new BeginCombo()/EndCombo() api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() items.
    // - The old Combo() api are helpers over BeginCombo()/EndCombo() which are kept available for convenience purpose.
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, bool owned = false);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
	IMGUI_API bool			MultiCombo(const char* name, const char** displayName, bool* data, int dataSize);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const items[], int items_count, int popup_max_height_in_items = -1, bool owned = false);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_max_height_in_items = -1, bool owned = false);
    ...
    IMGUI_API bool          ColorPicker4Fixed(const char* label, float& col0, float& col1, float& col2, float& col3, ImGuiColorEditFlags flags = 0, const float* ref_col = NULL);
	...
	IMGUI_API bool          Selectable2(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));
	IMGUI_API bool			Selectable3(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));
    ...
    IMGUI_API ImU32         ColorConvertFloat4ToU32Fixed(const ImVec4& in);
...
{
    ImVec4              value;
    //fixed colors for blur hm...
    //new rgb to bgr, so need to convert, here is convert...
   //ImColor() { value.x = value.y = value.z = value.w = 0.0f; }
   //ImColor(int r, int g, int b, int a = 255) { float sc = 1.0f / 255.0f; value.x = (float)b * sc; value.y = (float)g * sc; value.z = (float)r * sc; value.w = (float)a * sc; }
   //ImColor(ImU32 rgba) { float sc = 1.0f / 255.0f; value.x = (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc; value.y = (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc; value.z = (float)((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc; value.w = (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF) * sc; }
   //ImColor(float r, float g, float b, float a = 1.0f) { value.x = b; value.y = g; value.z = r; value.w = a; }
   //ImColor(const ImVec4& col) { value.x = col.z; value.y = col.y; value.z = col.x; value.w = col.w;    }
   //ImColor(float* col) { value.x = col[2]; value.y = col[1]; value.z = col[0]; value.w = col[3]; }
    //but maybe no, another way xd
	...
	ImColor(float* col)												{ value.x = col[0]; value.y = col[1]; value.z = col[2]; value.w = col[3]; }
...
struct ImDrawList
{
	...
	IMGUI_API void  AddConvexPolyFilled(const ImVec2* points, const int points_count, ImU32 col, ImU32 col2);
