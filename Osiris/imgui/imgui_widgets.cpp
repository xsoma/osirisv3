...
bool ImGui::ButtonTabsEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::ButtonTabsEx2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
{
	return ButtonTabsEx(label, size_arg, 0);
}

bool ImGui::ButtonTabsNormal(const char* label, const ImVec2& size_arg)
{
	return ButtonTabsEx2(label, size_arg, 0);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    //const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + 1.f));
	const ImRect total_bb(pos - 4, pos + ImVec2(window->Size.x - (pos.x - window->Pos.x) * 2.f, label_size.y + 1.f) + 4);
    ItemSize(total_bb, style.FramePadding.y);
	...
	ImColor hovered_color = g.Style.Colors[ImGuiCol_Border];

	RenderFrame(total_bb.Min, total_bb.Max, hovered_color, true, 2.f);
    ...
    {
		window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, check_col, 2.f);
        //const float pad = ImMax(1.0f, (float)(int)(square_sz / 6.0f));
		//RenderCheckMark(check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad*2.0f);
		//window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), check_col, check_col, check_col & ~IM_COL32_A_MASK, check_col & ~IM_COL32_A_MASK);
    }

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + 0.5f), label);
...
bool ImGui::ColorEditFixed(const char* label, float& v0, float& v1, float& v2, float& v3 )
{


    v0 = std::clamp(v0, 0.00f, 1.00f);
    v1 = std::clamp(v1, 0.00f, 1.00f);
    v2 = std::clamp(v2, 0.00f, 1.00f);
    v3 = std::clamp(v3, 0.00f, 1.00f);
    ...
    ImVec4 col_rgb = ImColor(v2,v1,v0,v3);
    ImVec4 col_rgb_without_alpha(col_rgb.x, col_rgb.y, col_rgb.z, 1.0f);
    float grid_step = ImMin(square_sz, square_sz) / 2.99f;
    float rounding = ImMin(2.f, grid_step * 0.5f);
    ImRect bb_inner = check_bb;
    float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
    bb_inner.Expand(off);
    // Because GetColorU32() multiplies by the global style Alpha and we don't want to display a checkerboard if the source code had no alpha
    ImVec4 col_source = col_rgb;
    if (col_source.w < 1.0f)
        RenderColorRectWithAlphaCheckerboard(check_bb.Min, check_bb.Max, GetColorU32(col_source), grid_step, ImVec2(0, 0), 2.f, ImDrawCornerFlags_All);
    else
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(col_source), 2.f, ImDrawCornerFlags_All);
    RenderNavHighlight(total_bb, id);

    if (pressed)
    {
        ImRect LastItemRect = GImGui->CurrentWindow->DC.LastItemRect;
        GImGui->ColorPickerRef = ImColor(v2, v1, v0, v3);

        ImGui::OpenPopup(label);

        ImVec2 rect_bottom_left = LastItemRect.GetBL();
        ImVec2 window_pos = ImVec2(rect_bottom_left.x - 1.f, rect_bottom_left.y + GImGui->Style.ItemSpacing.y);

        ImGui::SetNextWindowPos(window_pos);
    }

    ImVec2 PrevWindowPadding = style.WindowPadding;
    GImGui->Style.WindowPadding = ImVec2(8, 8);
    ...
    if (ImGui::BeginPopupModal(label, nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoBackground))
    {


        const auto flags = ImGuiColorEditFlags_PickerHueBar | ImGuiColorEditFlags_NoDragDrop |
            ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoSidePreview;

        ImGui::Spacing();
        ImGui::ColorPicker4Fixed("##picker", v2, v1, v0, v3, flags, &GImGui->ColorPickerRef.x);
        ImGui::Spacing();
        ...
        ImGui::DrawRectangleShadowVerticesAdaptive(shadow_settings, window_rect.Min, window_rect.Max);
    ...
    g.Style.WindowPadding = PrevWindowPadding;
...
bool ImGui::ColorEdit(const char* label, float* v)
{

    

	for (size_t i = 0; i < 4; i++)
		v[i] = std::clamp(v[i], 0.00f, 1.00f);
	...
	{
		ImRect LastItemRect = GImGui->CurrentWindow->DC.LastItemRect;
		GImGui->ColorPickerRef = ImColor(v);
	...
	if (ImGui::BeginPopupModal(label, nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoBackground))
	{
		...
		ImGui::Spacing();
		ImGui::ColorPicker4("##picker", v, flags, &GImGui->ColorPickerRef.x);
		ImGui::Spacing();
...
bool ImGui::KeyButton(const char* label, void* v, bool owned)
{
	static int keynum;
    ...
    std::string keystring = "insert key in src";
...
#include "../InputUtil.h"
bool ImGui::KeyButtonToggle(const char* label, void* v, bool owned)
{
	static int keynum;

    auto bind = reinterpret_cast<KeyBind*>(v);
	//int* bind = reinterpret_cast<int*>(v);
	...
	std::string keystring = bind->toString();
    //std::string keystring = "key in src";
	std::string popup_label = label;
	popup_label.append("##KeyAssignment");
...
bool ImGui::KeyButtonPress(const char* label, void* v, bool owned)
{
	static int keynum;
...
void ImGui::Separator(int count)
{
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool owned)
{
	...
	const float w = window->Size.x - (window->DC.CursorPos.x - window->Pos.x) * 2.f;
	const float renderframe_padding = label_size.x > 0.f ? 6 + label_size.y : 0.f;

	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, renderframe_padding), window->DC.CursorPos + ImVec2(w, renderframe_padding + (owned ? label_size.y + 1.f : 17.f)));
	const ImRect total_bb(window->DC.CursorPos - 4, frame_bb.Max + 4);
    ItemSize(total_bb, style.FramePadding.y);
	...
	if (label_size.x > 0)
		RenderText(ImVec2(total_bb.Min.x + 4, total_bb.Min.y + 4), label);

    if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), 2.f, ImDrawCornerFlags_All);
 ...
 //       if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
	//	{
	//		RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.f);
	//	}
 //   }

    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(ImVec2(frame_bb.Min.x + 4.f, frame_bb.Min.y), frame_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f,0.5f));
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(style.FramePadding.x, 4));
	PushStyleVar(ImGuiStyleVar_PopupRounding, 2.f);
...
void ImGui::EndCombo()
{
	...
	shadowSettings.shadowOffset = ImVec2(0, 5);
...
bool ImGui::MultiCombo(const char* name, const char** displayName, bool* data, int dataSize) 
{
	...
	if (ImGui::BeginCombo(name, previewText)) {
		...
		for (int i = 0; i < dataSize; i++) {
			...
			if (ImGui::Selectable3(buf, data[i], ImGuiSelectableFlags_DontClosePopups)) {
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, bool owned)
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, bool owned)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, owned);
    return value_changed;
}
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 17));
    ...
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, 2.f);
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (!is_decimal && v_range >= 0)                                             // v_range < 0 may happen on integer overflows
        grab_sz = style.GrabMinSize;  // For integer sliders: if possible have the grab size represent 1 unit
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImGuiID id = window->GetID(label);
	const float w = window->Size.x - (window->DC.CursorPos.x - window->Pos.x) * 2.f;
	const float renderframe_padding = 6 + g.FontSize;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, renderframe_padding), window->DC.CursorPos + ImVec2(w, renderframe_padding + 6));
    const ImRect total_bb(window->DC.CursorPos - 4, ImVec2(frame_bb.Max.x, frame_bb.Max.y + 5 + g.FontSize) + 4);
	...
	bool held;
	ButtonBehavior(total_bb, id, nullptr, &held);
    ...
    // Draw frame
	const ImU32 frame_col = GetColorU32(ImGuiCol_FrameBg);//GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 2.f);
	
    // Slider behavior
	ImRect grab_bb;
	const auto backup_grabminsize = style.GrabMinSize;
	g.Style.GrabMinSize = 0.f;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, v, v_min, v_max, format, power, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
	g.Style.GrabMinSize = backup_grabminsize;

	if (grab_bb.Max.x - frame_bb.Min.x > 0.f)
		RenderFrame(frame_bb.Min, ImVec2(grab_bb.Max.x, frame_bb.Max.y), GetColorU32(ImGuiCol_FrameBgActive), false, 2.f);
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Max.y + 5), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 5 + g.FontSize), value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? GetTextLineHeight() * 8.0f : 17)); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
	...
	ImColor color_top = GetColorU32(ImGuiCol_FrameBg);
	ImColor color_bottom = ImColor(color_top.value.x * 0.500f, color_top.value.y * 0.500f, color_top.value.z * 0.500f, color_top.value.w);
	ImColor color_border = ImColor(0.039f, 0.039f, 0.039f, color_top.value.w);
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(3, 2);
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + 0.5f), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        static const char* fmt_table_int[3][4] =
        {
            ...
            { "B:%3d", "G:%3d", "R:%3d", "A:%3d" }, // Long display for RGBA
        ...
        value_changed |= DragInt(ids[2], &i[2], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][2]);
        SameLine(0, style.ItemInnerSpacing.x);
        SetNextItemWidth(w_item_one);
        value_changed |= DragInt(ids[1], &i[1], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][1]);
        SameLine(0, style.ItemInnerSpacing.x);
        SetNextItemWidth(w_item_last);
        value_changed |= DragInt(ids[0], &i[0], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][0]);
...
bool ImGui::ColorPicker4Fixed(const char* label, float& col0, float& col1, float& col2, float& col3, ImGuiColorEditFlags flags, const float* ref_col)
{
    float col[4] = { col0, col1, col2, col3 };
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float square_sz = 15.f;//GetFrameHeight();
    float bars_width = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = 200.f;//ImMax(bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_NoSmallPreview))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hex", col, sub_flags | ImGuiColorEditFlags_DisplayHex | ImGuiColorEditFlags_NoSmallPreview);
        PopItemWidth();
    }
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, col32_no_alpha, 25);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, IM_COL32_WHITE, 25);
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)", cr, cg, cb, cr, cg, cb, col[2], col[1], col[0]);
        else
            Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cr, cg, cb, ca, cr, cg, cb, ca, col[2], col[1], col[0], col[3]);
    }
    else if (flags & ImGuiColorEditFlags_InputHSV)
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
...
bool ImGui::Selectable2(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	// Selectables are tightly packed together so we extend the box to cover spacing between selectable.
	const float spacing_x = style.ItemSpacing.x - 4.f;
	const float spacing_y = style.ItemSpacing.y - 2.f;
	const float spacing_L = (float)(int)(spacing_x * 0.50f);
	const float spacing_U = (float)(int)(spacing_y * 0.50f);
	bb.Min.x -= spacing_L;
	bb.Min.y -= spacing_U;
...
bool ImGui::Selectable3(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, window->Size.x - WINDOW_WIDGETS_PADDING_SAMELINE * 2.f, GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    while (clipper.Step())
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        {
            ...
            PushID(i);
            if (Selectable2(item_text, item_selected))
            {
                *current_item = i;
                value_changed = true;
            }
