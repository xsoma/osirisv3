...
ImU32 ImGui::ColorConvertFloat4ToU32Fixed(const ImVec4& in)
{
    ImU32 out;
...
void ImGui::RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale)
{
    ...
    case ImGuiDir_Down:
        ...
        a = ImVec2(+0.000f,+0.633f) * r;
        b = ImVec2(-0.633f,-0.500f) * r;
        c = ImVec2(+0.633f,-0.500f) * r;
        break;
    case ImGuiDir_Left:
    case ImGuiDir_Right:
        if (dir == ImGuiDir_Left) r = -r;
        a = ImVec2(+0.500f,+0.000f) * r;
        b = ImVec2(-0.500f,+0.633f) * r;
        c = ImVec2(-0.500f,-0.633f) * r;
        break;
    case ImGuiDir_None:
    case ImGuiDir_COUNT:
        IM_ASSERT(0);
...
void ImGui::UpdateMouseWheel()
{
	//ImGuiContext& g = *GImGui;
    //
	//static bool smooth_scrolling;
	//static bool interpolated;
    //
	//static float interpolate_time;
	//static float scroll_start;
	//static float scroll_end;
    //
	//static int HoveredWindowID;
    //
	//const auto scroll_step = g.IO.MouseWheel;
    //
	//if (g.HoveredWindow)
	//{
	//	if (HoveredWindowID != g.HoveredWindow->ID)
	//	{
	//		smooth_scrolling = false;
	//		interpolated = false;
	//		scroll_start = 0.f;
	//		scroll_end = 0.f;
	//		HoveredWindowID = g.HoveredWindow->ID;
	//	}
	//}
    //
	//if (!smooth_scrolling && scroll_step != 0.f && g.HoveredWindow && !g.HoveredWindow->Collapsed)
	//{
    //    //client_state->time 
    //    //interpolate_time = static_cast<float>(client_state->time);
	//	interpolate_time = static_cast<float>(0);
	//	smooth_scrolling = true;
	//}
    //
	//if (g.HoveredWindow && !g.HoveredWindow->Collapsed && smooth_scrolling)
	//{
    ...
    //
	//		if (!interpolated && scroll_step != 0.f)
	//		{
	//			scroll_start = scroll_window->Scroll.y;
	//			scroll_end = scroll_start - scroll_step * scroll_amount;
    //
	//			if (scroll_end < 0.f)
	//			{
	//				if (scroll_window->Scroll.y <= 0.f)
	//					return;
    //
	//				scroll_end = 0.f;
	//			}
	//			else if (scroll_end > scroll_window->ScrollMax.y)
	//			{
	//				if (scroll_window->Scroll.y >= scroll_window->ScrollMax.y)
	//					return;
    //
	//				scroll_end = scroll_window->ScrollMax.y;
	//			}
	//			else
	//				interpolated = true;
	//		}
	//		else if (interpolated && scroll_step != 0.f)
	//		{
	//			scroll_start = scroll_window->Scroll.y;
	//			scroll_end -= scroll_step * scroll_amount;
    //
	//			if (scroll_end < 0.f)
	//			{
	//				if (scroll_window->Scroll.y <= 0.f)
	...
	//		}
    //
	//		//auto scroll_lerp = static_cast<float>(round(Math::Interp(interpolate_time,
	//		//	client_state->time, interpolate_time + 0.100f, scroll_start, scroll_end)));
    //        auto scroll_lerp = static_cast<float>(round(Interp(interpolate_time,
    //            0, interpolate_time + 0.100f, scroll_start, scroll_end)));
    //
	//		SetScrollY(scroll_window, scroll_lerp);
    //
	//		//if (interpolate_time + 0.100f <= static_cast<float>(client_state->time))
    //        if (interpolate_time + 0.100f <= static_cast<float>(0))
	//		{
	//			smooth_scrolling = false;
	//			interpolated = false;
	//			scroll_start = 0.f;
...
bool ImGui::BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
    ...
    PushStyleColor(ImGuiCol_ChildBg, style.Colors[ImGuiCol_PopupBg]);
...
void ImGui::EndChildFrame()
{
	...
	{
		ImColor color_framebg = GetStyleColorVec4(ImGuiCol_FrameBg);

		ImColor color_top = ImColor(20, 20, 20, 100);
		ImColor color_bottom = color_top;
		color_bottom.value.w = 0.00f;

		if (window->Scroll.y > 1.0f)
			window->DrawList->AddRectFilledMultiColor(minTop, maxTop, color_top, color_top, color_bottom, color_bottom);

		if ((GetScrollMaxY() - window->Scroll.y) > 1.f)
			window->DrawList->AddRectFilledMultiColor(minBottom, maxBottom, color_bottom, color_bottom, color_top, color_top);
	}
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
	...
	{
		ImColor border_color = GetColorU32(ImGuiCol_Border);
		border_color.value.w = 0.25f;
		window->DrawList->AddRect(window->Pos, window->Pos + window->Size, border_color, rounding, ImDrawCornerFlags_All, border_size);
	}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
		...
		else
		{
			...
			if (alpha != 1.0f)
				bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
			window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size - ImVec2(0, 4), bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);		
			window->DrawList->AddRectFilled(ImVec2(window->Pos.x, window->Pos.y + window->Size.y - 4), window->Pos + window->Size, GetColorU32(ImGuiCol_Tab));
		}
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
			...
			window->DrawList->AddRectFilled(ImVec2(title_bar_rect.Min.x, title_bar_rect.Max.y), title_bar_rect.Max + ImVec2(0, 4), GetColorU32(ImGuiCol_TabActive), window_rounding, ImDrawCornerFlags_Top);
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
	...
	ImGui::PushFont(g_pFontList[Verdana_15px]);
...
bool ImGui::BeginDefault(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    window->DC.NavFocusScopeIdCurrent = (flags & ImGuiWindowFlags_ChildWindow) ? parent_window->DC.NavFocusScopeIdCurrent : 0; // Inherit from parent only 
