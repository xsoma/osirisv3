...
struct IMGUI_API ImRect
{
	...
	void		Clear()								{ Min.x = 0.f; Min.y = 0.f; Max.x = 0.f, Max.y = 0.f; }
};
...
public:
    ...
    float       TitleBarHeight() const      { ImGuiContext& g = *GImGui; return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.0f : CalcFontSize() + 10.f; }
...
namespace ImGui
{
	...
	IMGUI_API bool          ButtonTabsEx(const char* label, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0);
	IMGUI_API bool          ButtonTabsEx2(const char* label, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0);
